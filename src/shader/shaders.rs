...
pub fn ambient(intersection: &Intersection) -> Vector {
    intersection.triangle.material().ambient
}

pub fn emittance(intersection: &Intersection) -> Vector {
    intersection.triangle.material().emittance
}

pub fn map_uv(intersection: &Intersection) -> TextureCoordinate{
    let texa = intersection.triangle.texture_a();
    let texb = intersection.triangle.texture_a();
    let texc = intersection.triangle.texture_a();

    let e1 = texc-texa;
    let e2 = texb-texa;

    texa.to_owned() + (e1 * intersection.uv.0) + (e2 * intersection.uv.1)
}
...
pub fn specular(intersection: &Intersection, hit_pos: Vector, light_pos: Vector, cam_pos: Vector) -> Vector {
    ...
    spec.powf(triangle.material().shininess) * triangle.material().specular
}